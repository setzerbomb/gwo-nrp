...
void computeWolfMoves (int i, int dim, double a, Wolf *population, Wolf *alpha_wolf, Wolf *beta_wolf, Wolf *gamma_wolf) {
    double X1[dim];
    double X2[dim];
    double X3[dim];
    double nextPos[dim];
        ...
        #pragma omp single
        {
            while (iter < (max_iter * max_iter_multiplier) && (max_iter_multiplier < 2)) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    computeWolfMoves(i, dim, a, population, alpha_wolf, beta_wolf, gamma_wolf);
                }
                #pragma omp taskwait
...
int main() {
    ...
    */
    ...
    #pragma omp parallel
    {
        #pragma omp for schedule(static) reduction(+:total_sp) reduction(+:total_satisfaction)
        for (int i = 0; i < dim; i++) {
